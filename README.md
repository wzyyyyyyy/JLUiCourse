...
界面左侧的「捡漏模式」勾选框切换两种重试策略，默认关闭（保持原有行为）：

| | 重试间隔 | 遇到「课容量已满」 | 适用场景 |
| --- | --- | --- | --- |
| 默认（开抢） | 40-100 ms | 判定为永久失败，放弃该门课 | 正选开放的瞬间 |
| 捡漏模式 | 200-500 ms | 继续重试，等待他人退课 | 退补选期间长时间挂机 |

勾选状态从下一次重试开始生效，运行中也可以切换。捡漏模式下课程不会因为满员而结束，
需要手动点「停止选课」。长时间挂机请注意登录态可能过期。
